...
impl QueryCacheKey {
    pub fn from_descriptor_with_version(desc: &lance_distributed_proto::descriptor::LanceQueryDescriptor, dataset_version: u64) -> Self {
        let vector_hash = if let Some(ref vq) = desc.vector_query {
...
mod tests {
    ...
    async fn test_cache_ttl_expiry() {
        ...
        let key = QueryCacheKey {
            table_name: "t".into(), query_type: 0, vector_hash: 1, filter: None, k: 5, dataset_version: 1,
        };
    ...
    async fn test_cache_max_entries() {
        ...
        for i in 0..3 {
            let key = QueryCacheKey {
                table_name: "t".into(), query_type: 0, vector_hash: i, filter: None, k: 5, dataset_version: 1,
            };
    ...
    async fn test_cache_disabled() {
        ...
        let key = QueryCacheKey {
            table_name: "t".into(), query_type: 0, vector_hash: 0, filter: None, k: 5, dataset_version: 1,
        };
        cache.put(key.clone(), make_batch(vec![1])).await;
