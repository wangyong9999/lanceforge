...
// Edge case and robustness tests for Lance-Ballista integration.
...
use ballista_lance::codec::proto::{LanceQueryDescriptor, LanceQueryType, VectorQueryParams, FtsQueryParams};
use ballista_lance::config::{ClusterConfig, ExecutorConfig, ShardConfig, TableConfig};
use ballista_lance::executor::LanceTableRegistry;
use ballista_lance::merge::{global_top_k_by_distance, global_top_k_by_score, reciprocal_rank_fusion};
...
fn test_ipc_empty_data_error() {
    let result = ballista_lance::executor::ipc_to_record_batch(&[]);
    assert!(result.is_err());
}

#[test]
fn test_ipc_corrupt_data_error() {
    let result = ballista_lance::executor::ipc_to_record_batch(&[0xFF, 0xFE, 0xFD]);
    assert!(result.is_err());
}
...
fn test_ipc_roundtrip() {
    ...
    let ipc = ballista_lance::executor::record_batch_to_ipc(&batch).unwrap();
    let decoded = ballista_lance::executor::ipc_to_record_batch(&ipc).unwrap();
