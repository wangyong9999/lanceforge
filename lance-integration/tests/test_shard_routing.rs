...
use ballista_lance::shard_pruning::{ColumnStats, ShardMetadata, ShardPruner};
...
async fn test_shard_pruning_in_cluster() {
    ...
    if resp.num_rows > 0 {
        let batch = ballista_lance::executor::ipc_to_record_batch(&resp.arrow_ipc_data).unwrap();
